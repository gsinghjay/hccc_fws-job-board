...
if(str_replace('\\', '/', strtolower(__FILE__)) == str_replace('\\', '/', strtolower($_SERVER['SCRIPT_FILENAME'])) && isset($_GET['datasource']) && $_GET['datasource'] != ''){
	get_calevents_dmc_output(array('endpoint' => true));
}

function get_calevents_dmc_output($options){
	global $eventsDMC;
...
class EventsDMC {
	...
	public function render_homepage_events_list($resultSet){
		...
		if (count($items) >= 1) {
			foreach ($items as $i) {
				$calendar = $i->children('cal', true)->calendar;
				$guid = $i->children('cal', true)->guid;
				$start = strtotime($i->children('cal', true)->start);
				$end = strtotime($i->children('cal', true)->end);
				$recurring = $i->children('cal', true)->recurring;
				$featured = $i->children('cal', true)->featured;
				$location = $i->children('cal',true)->location;
				$title = $i->title;
				$link = $i->link;
				$month = date("M", $start);
				$day = date("d", $start);
				...
				if(preg_match('/[TZ]+/', $i->children('cal', true)->start)){
