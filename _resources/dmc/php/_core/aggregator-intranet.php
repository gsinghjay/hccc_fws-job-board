...
// Aggregator script for aggregating DMC xml files in the intranet site.
// We can't use the aggregator script on the intranet site because it is behind a login.

// 	error_reporting(E_ALL);
// 	ini_set('display_errors', 1);

$_SERVER['INTRANET_ROOT'] = 'C:\inetpub\wwwroot\clients\inside.fandm.edu';
...
function get_xml_paths_from_aggregation_paths($aggregation_paths){
	...
	foreach($aggregation_paths as $path){
		...
		$paths = get_xml_paths($_SERVER['INTRANET_ROOT'] . $path);
