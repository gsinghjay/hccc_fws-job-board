...
<xsl:stylesheet version="3.0" 
				...
				expand-text="yes"
				>
	<xsl:import href="variables.xsl"/> <!-- customer variables -->
<!-- 	<xsl:param name="ou:navigation-end" ></xsl:param> -->
	...
	<xsl:template name="nav-builder">
		...
		<xsl:param name="top-level" as="xs:string">
			<xsl:choose>
				<xsl:when test="$ou:navigation-end !=''">{$ou:navigation-end}</xsl:when>
				<xsl:otherwise>/</xsl:otherwise>
			</xsl:choose>
		</xsl:param>
		<xsl:param name="path" as="xs:string">{$ou:dirname}</xsl:param>
	...
	<xsl:template name="nav">
		...
		<ul data-nav-path="{$href}{$section-index-filename}" >
