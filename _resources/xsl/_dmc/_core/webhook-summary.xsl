		...
		<html lang="en">
			...
			<body id="properties">
				<div class="container" style="max-width: 800px; margin: 20px 0 0 20px;">
					<h1>Webhook Listener Configuration</h1>
					<p>
						This will publish to: <xsl:value-of select="$domain || $dirname || replace($ou:filename, '\..*$', if($server-type = 'php') then '.php' else '.ashx')" />
					</p>
					<h2>Settings</h2>
					<dl>
						<xsl:apply-templates select="/document/ouc:properties//parameter"/>
					</dl>
				</div>
