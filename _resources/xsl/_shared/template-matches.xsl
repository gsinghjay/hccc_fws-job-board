...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:ou="http://omniupdate.com/XSL/Variables" xmlns:ouc="http://omniupdate.com/XSL/Variables" expand-text="yes"
	...
	<xsl:template match="table[not(@data-snippet)]">
		...
		<div class="table-block">
			<div class="table-block table-block__holder">
				<!--<div class="table-responsive">-->
					<table>
						<xsl:attribute name="class">{string-join(($class, ''), ' ')}</xsl:attribute>
						<xsl:apply-templates select="node()"/>
					</table>
				<!--</div>-->
				<span class="table-block table-block__notification">Slide for more</span>
			</div>
		</div>
	</xsl:template>
	
	<!-- if a table with the class ou-snippet-column-content is found, it will exclude images from the table -->
	<xsl:template match="table[@class='ou-snippet-column-content']//img" />
